/* -*- P4_16 -*- */
/*This is the TCP-based HULApp code version.
  ...
  4. Added TCP parsing
  5. Added Application-Aware logic: based on the TCP dst port in the packet header, HULApp takes the packet next hop from one of the following tables:
        HULApp_nhop_QDepth (if dst TCP port belongs to the port range defined for latency-sensitive application group)
        HULApp_nhop_pathUtil (if dst TCP port belongs to the port range defined for less latency-sensitive application group))
*/
#include <core.p4>
#include <v1model.p4>
...
typedef bit<22> qdepth_t; // header type in the probe which is used for latecy sensitive traffic metric
typedef bit<39> path_util_t; // header type in the probe which is used for less latecy sensitive traffic metric
typedef bit<32> digest_t;
typedef bit<48> last_seen_t; 
...
header hulapp_t {
    ...
    path_util_t path_util; //metric for path_util (pps)
    /* digest of the source routing list to uniquely identify each path */
    digest_t digest;
    /* : flags indicating which best path at dest ToR has been changed: for latency/non-lat sensitive traffic, or for both  */
...
header tcp_t {//tcp header structure
	bit<16> srcPort;
    bit<16> dstPort;
    bit<32> seqNo;
    bit<32> ackNo;
...
parser MyParser(packet_in packet,
    ...
    state parse_ipv4 {
        ...
        transition select(hdr.ipv4.protocol) {
            8w17: parse_udp;
            8w6 : parse_tcp; //triggers parsing tcp
            default: accept;
        }
    }
...
control MyIngress(inout headers hdr,
     ...
     * each source ToR
     */
    register<path_util_t>(TOR_NUM) srcindex_path_util_reg; // path utilization register
    ...
    /* At each hop, saves the next hop to reach each destination ToR (for non latency-sensitive traffic) */
    ...
    /*: at each hop saves the timestamp when the last time flowlet was seen */
     ...
     */
    action hulapp_set_nhop_q(bit<32> index) { //: set next hop for latency sensitive apps
    	dstindex_nhop_reg_q.write(index, (bit<16>)standard_metadata.ingress_port);
    }

    action hulapp_set_nhop_u(bit<32> index) { //: set next hop for non-latency sensitive apps
    ...
    action hulapp_get_nhop_q(bit<32> index){ //: read next hop for latency sensitive apps
       	bit<16> tmp;
       	dstindex_nhop_reg_q.read(tmp, index); 
       	standard_metadata.egress_spec = (bit<9>)tmp; 
  	} 

  	action hulapp_get_nhop_u(bit<32> index){ //: read next hop for non latency sensitive apps
  		bit<16> tmp;
    ...
    table hulapp_bwd_u { //: table for updating best paths for non latency sensitive applications (based on link util)
        key = {
            hdr.ipv4.dstAddr: ternary;
        }
        actions = {
    ...
    apply {
        if (hdr.hulapp.isValid()){
            if (hdr.hulapp.dir == 0){
                switch(hulapp_fwd.apply().action_run){
                    ...
                    hulapp_dst: {
                        ...
                        /* : checking, if any of metrics has changed and updatin the best path at dst ToR */
                        if ((old_qdepth > hdr.hulapp.qdepth) || (old_path_util > hdr.hulapp.path_util)){
                        	...
                        	if (old_path_util > hdr.hulapp.path_util){ // condition and logic for updating path_util 
        ...
        }else if (hdr.ipv4.isValid()){//: adding logic for processing flowlets from different applications (latancy sensitive and less latency sensitive)
            bit<16> flowlet_hash;
            hash(
                flowlet_hash, 
                HashAlgorithm.crc16, 
                16w0, 
                { hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.ipv4.protocol, hdr.tcp.srcPort, hdr.tcp.dstPort}, //: updated hash to 5 tuple (+tcp src/dst ports)
                32w65536);

            /* look into hulapp tables */
            bit<16> port;
            bit<48> timestamp; //: flowlet timestamp
            ...
            if ((port == 0) || ((standard_metadata.ingress_global_timestamp - timestamp) > ipg)) {
                ...
                if (((hdr.tcp.dstPort >= 31)&&(hdr.tcp.dstPort <= 51))||((hdr.tcp.srcPort >= 31)&&(hdr.tcp.srcPort <= 51))){
                	hulapp_nhop_qDepth.apply(); //next hop for lat sens app WS
                }
                if (((hdr.tcp.dstPort >= 1)&&(hdr.tcp.dstPort <= 21))||((hdr.tcp.srcPort >= 1)&&(hdr.tcp.srcPort <= 21))){
                	hulapp_nhop_pathUtil.apply(); //next hop for non lat sens app DM
                }
...
control MyEgress(inout headers hdr,
    ...
    apply {
        if (hdr.hulapp.isValid() && hdr.hulapp.dir == 0){
            ...
            /* estimating dequeue time delta to the closest power of 2*/
            bit<32> delta;
            ...
            /* receiving link_util metric: number of packet sent divided by timedelta */
            bit<39> tx_util;
            if (delta == (1 << 8)) {
